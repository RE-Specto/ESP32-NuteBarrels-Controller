...
#define FILLING_STATE 0x20  // F = filling task on
#define MIXING_STATE 0x10   // M = mixing task on
#define STORING_STATE 0x08  // S = storing task on
#define DRAINING_STATE 0x04 // D = draining task on
#define BYPASS_STATE 0x02   // B = bypass task on
...
struct sSystem
{
    ...
    // D = Draining state
    // X = stopped state
    byte _state_now = FILLING_STATE + STOPPED_STATE ; // 00100001 - filling + waiting for nutes - initial system state
...
public:
    ...
    // void BypassRecalc(byte pool);
    uint32_t DrainTotal();
    uint32_t BypassTotal();
    void SetManual(byte task, byte src, byte dest, uint16_t ammo);
    void DecreaseManual(uint32_t  liters);
    void DecreaseBypass(uint32_t  liters);
